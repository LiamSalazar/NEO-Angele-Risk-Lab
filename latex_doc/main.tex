...
\hypersetup{
  ...
  pdfauthor={Salazar Martínez Liam Antonio y Vargas Nicolás Bianca Celeste}
}
...
{\Large Vargas Nicolás Bianca Celeste\par}
\vfill
...
La inspección del repositorio actual verificó código Python, pruebas, documentación, reportes, artefactos Parquet, diagramas Mermaid, una figura UML en \codepath{artifacts/figures/class\_diagram\_entities.png}, backend FastAPI, frontend React/TypeScript/Vite y configuración Docker Compose. La validación local encontró que Docker no está disponible en la computadora usada para preparar esta entrega, por lo que no se compiló PDF ni se levantaron servicios; el repositorio sí contiene \codepath{docker-compose.yml}, \codepath{Dockerfile} y \codepath{Dockerfile.frontend}.
...
El proyecto se ubica en la asignatura Desarrollo de Aplicaciones para Análisis de Datos porque combina adquisición de datos, almacenamiento por capas, procesamiento con Spark, programación orientada a objetos, diseño de API, frontend, modelos de machine learning, grafo orbital, pruebas automatizadas, documentación técnica y empaquetamiento con Docker.
...
La justificación del score explicable proviene de la necesidad de ordenar objetos para revisión. Una probabilidad oficial de impacto requiere efemérides, observación profesional y sistemas especializados; el proyecto no posee ni reclama esa capacidad. En cambio, calcula un score experimental que combina tamaño, proximidad orbital, acercamientos, señal Sentry, incertidumbre y calidad de datos.
...
El proyecto usa fuentes públicas NASA/JPL documentadas oficialmente \cite{jpl_sbdb,jpl_sbdb_query,jpl_cad,jpl_sentry}. La conexión con el pipeline aparece en \codepath{src/neo\_ange/clients} y \codepath{src/neo\_ange/pipelines/ingestion.py}.
...
El diccionario siguiente agrupa los campos principales que aparecen en el dataset gold y en los reportes. No inventa columnas: los nombres fueron verificados en \codepath{data/gold/neo\_risk\_features}, \codepath{data/gold/risk\_scores}, \codepath{reports/model\_evidence} y módulos de código.

\begin{longtable}{>{\RaggedRight\arraybackslash}p{0.18\linewidth}>{\RaggedRight\arraybackslash}p{0.27\linewidth}>{\RaggedRight\arraybackslash}p{0.17\linewidth}>{\RaggedRight\arraybackslash}p{0.28\linewidth}}
...
El diagrama fuente está en \codepath{docs/diagrams/system\_architecture.mmd}. El repositorio también contiene diagramas Mermaid de scoring, Monte Carlo, simulación orbital, ML evidence, grafo GNN, navegación final, pipeline de datos y clases. El documento incluye el diagrama UML de entidades puras renderizado desde el repositorio.
...
Ruta & Propósito\\
\midrule
\codepath{src/neo\_ange/clients} & Clientes HTTP para fuentes NASA/JPL.\\
\codepath{src/neo\_ange/etl} & Lectura bronze, transformaciones silver y construcción gold.\\
\codepath{src/neo\_ange/domain} & Clases de dominio, factories, repositories y objetos analíticos.\\
\codepath{src/neo\_ange/risk} & Score, categorías, explicaciones, ranking, validación y reportes.\\
\codepath{src/neo\_ange/simulation} & Monte Carlo del score, incertidumbre y sensibilidad.\\
\codepath{src/neo\_ange/orbital\_simulation} & Elementos orbitales, covarianza, perturbación, propagación, métricas y servicio.\\
\codepath{src/neo\_ange/ml} & Datasets, feature sets, baselines, métricas, validación y leakage.\\
\codepath{src/neo\_ange/gnn} & Construcción de grafo, similitud, datasets, baselines, modelos GCN/GraphSAGE y reportes.\\
\codepath{src/neo\_ange/evidence} & Evidencia de modelos, predicciones, tarjetas y desacuerdos.\\
\codepath{src/neo\_ange/findings} & Hallazgos analíticos por módulo.\\
\codepath{src/neo\_ange/api} & Aplicación FastAPI y routers.\\
\codepath{frontend} & Aplicación React/TypeScript/Vite.\\
\codepath{data} & Capas bronze, silver, gold y resultados Parquet.\\
\codepath{reports} & Reportes JSON, CSV y Markdown generados.\\
\codepath{docs} & Documentación técnica y diagramas Mermaid.\\
\codepath{artifacts} & Figuras, capturas y exportaciones.\\
\bottomrule
\end{tabularx}
...
\codepath{src/neo\_ange/pipelines/ingestion.py} coordina clientes y \code{BronzeStorage}. \codepath{src/neo\_ange/etl/bronze\_reader.py} lee wrappers JSON. \codepath{src/neo\_ange/etl/silver\_transformers.py} transforma SBDB Object, CAD y Sentry. \codepath{src/neo\_ange/etl/gold\_builder.py} une tablas, calcula features derivadas y produce \codepath{data/gold/neo\_risk\_features}. \codepath{src/neo\_ange/spark/session.py} crea una sesión local de Spark.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/class_diagram_entities.png}
\caption{Diagrama UML de entidades puras del dominio, copiado desde \codepath{artifacts/figures/class\_diagram\_entities.png}.}
\end{figure}

El diagrama muestra \code{Asteroid} como agregado y sus componentes. \code{AsteroidIdentity} está separado porque los identificadores tienen reglas propias de selección: \code{object\_key}, \code{spkid}, \code{des}, \code{full\_name} y \code{name} no tienen la misma estabilidad. \code{CloseApproachHistory} y \code{CloseApproachSummary} coexisten conceptualmente porque un acercamiento individual y un resumen agregado responden a necesidades distintas; el código actual usa \code{CloseApproach} para un registro y \code{CloseApproachSummary} para el resumen usado en score y frontend. \code{SentryRiskSignal} es opcional porque la mayoría de objetos no tiene señal Sentry en el snapshot local.

\section{Clases Principales}

\begin{longtable}{>{\RaggedRight\arraybackslash}p{0.17\linewidth}>{\RaggedRight\arraybackslash}p{0.21\linewidth}>{\RaggedRight\arraybackslash}p{0.32\linewidth}>{\RaggedRight\arraybackslash}p{0.20\linewidth}}
...
\midrule
\endhead
\code{Asteroid} & \codepath{src/neo\_ange/domain/asteroid.py} & Agregado raíz con identidad, órbita, propiedades físicas, acercamientos, Sentry y etiquetas. & \code{object\_key}, \code{display\_name}, \code{has\_risk\_relevant\_data}, \code{to\_feature\_dict}, \code{to\_dict}.\\
\code{AsteroidIdentity} & \codepath{src/neo\_ange/domain/identity.py} & Identificadores estables y nombre legible. & \code{best\_identifier}, \code{display\_name}, \code{to\_dict}.\\
\code{Orbit} & \codepath{src/neo\_ange/domain/orbit.py} & Elementos orbitales y señales de calidad. & \code{has\_minimum\_orbital\_data}, \code{orbital\_vector}, \code{proximity\_indicator}, \code{uncertainty\_indicator}.\\
\code{PhysicalProperties} & \codepath{src/neo\_ange/domain/physical.py} & Magnitud absoluta, diámetro, albedo y log diámetro. & \code{size\_indicator}, \code{has\_size\_information}, \code{to\_dict}.\\
\code{CloseApproach} & \codepath{src/neo\_ange/domain/approach.py} & Un acercamiento individual. & \code{distance\_indicator}, \code{velocity\_indicator}, \code{to\_dict}.\\
\code{CloseApproachSummary} & \codepath{src/neo\_ange/domain/approach.py} & Resumen CAD por objeto. & \code{has\_close\_approach\_data}, \code{approach\_priority\_indicator}.\\
\code{SentryRiskSignal} & \codepath{src/neo\_ange/domain/sentry.py} & Señales Sentry disponibles. & \code{has\_sentry\_signal}, \code{sentry\_priority\_indicator}, \code{to\_dict}.\\
\code{AsteroidFactory} & \codepath{src/neo\_ange/domain/factories.py} & Construye entidades desde filas gold, risk y simulación. & \code{from\_gold\_row}, \code{from\_risk\_row}, \code{many\_from\_dataframe}, \code{risk\_score\_from\_row}.\\
\code{GoldFeatureRepository}, \code{RiskScoreRepository}, \code{SimulationResultRepository} & \codepath{src/neo\_ange/domain/repositories.py} & Aíslan lectura de Parquet y devuelven entidades. & \code{load\_dataframe}, \code{load\_asteroids}, \code{get\_by\_object\_key}, \code{top}.\\
\bottomrule
\end{longtable}

\section{Extractos Exactos}

\begin{lstlisting}[language=Python,caption={Extracto de src/neo_ange/domain/asteroid.py}]
...
\begin{lstlisting}[language=Python,caption={Extracto de src/neo_ange/domain/factories.py}]
...
\input{tables/code_inventory.tex}
...
El patrón Repository aísla persistencia. \code{GoldFeatureRepository} sabe dónde está \codepath{data/gold/neo\_risk\_features}; \code{RiskScoreRepository} sabe que la ruta preferida es \codepath{data/gold/risk\_scores/risk\_scores.parquet}; \code{SimulationResultRepository} sabe cómo leer resultados Monte Carlo. Los routers y servicios no manipulan rutas Parquet directamente cuando trabajan con dominio.
...
El \emph{Risk Priority Score} es un score experimental, reproducible y explicable. No es probabilidad oficial de impacto. Se implementa en \codepath{src/neo\_ange/risk/scoring.py} y usa constantes de \codepath{src/neo\_ange/risk/schemas.py}. La versión actual verificada es \code{risk-score-v0.1.0}.
...
\begin{tabularx}{\linewidth}{>{\RaggedRight\arraybackslash}p{0.28\linewidth}>{\RaggedRight\arraybackslash}p{0.16\linewidth}Y}
\toprule
...
\begin{lstlisting}[language=Python,caption={Pesos exactos en src/neo_ange/risk/schemas.py}]
...
\begin{lstlisting}[language=Python,caption={Variables base en src/neo_ange/simulation/uncertainty.py}]
...
En el snapshot verificado, \codepath{data/gold/simulation\_results/score\_uncertainty\_results.parquet} contiene 221 filas y 107 objetos únicos. El resumen JSON reporta \code{fallback\_used\_count=10} y \code{formal\_uncertainty\_count=0}, lo cual confirma que las simulaciones no deben presentarse como incertidumbre física calibrada.

\chapter{Simulación Orbital Monte Carlo}

La simulación orbital se implementa en \codepath{src/neo\_ange/orbital\_simulation}. El servicio usa \code{n\_clones=300}, \code{horizon\_days=3650}, \code{time\_step\_days=10} y \code{random\_state=42} por defecto. Esos valores equilibran costo, resolución temporal y rapidez para API/frontend: diez años de horizonte con pasos de diez días da una exploración suficiente para visualización experimental, pero no sustituye efemérides oficiales.
...
\begin{lstlisting}[language=Python,caption={Fallback y parámetros en src/neo_ange/orbital_simulation/monte_carlo.py}]
def simulate_object(
    self,
    row: dict[str, Any],
    n_clones: int = 300,
...
\begin{lstlisting}[language=Python,caption={Peso de incertidumbre orbital en src/neo_ange/orbital_simulation/perturbation.py}]
return float(
...
La propagación usa un modelo heliocéntrico aproximado de dos cuerpos. Para cada clon, se avanza la anomalía media con \(M(t)=M_0+n t\). La ecuación de Kepler es:
...
El parámetro real por defecto es \code{k=10}. En el snapshot actual, \codepath{data/gold/gnn\_graph/nodes.parquet} contiene 1,000 nodos y \codepath{edges.parquet} contiene 6,955 aristas. El reporte \codepath{reports/findings/findings\_summary.json} conserva una corrida de 4,000 nodos y 27,829 aristas; se mantiene como reporte existente, pero no se mezcla con el grafo actual.
...
Los findings convierten artefactos analíticos en conclusiones legibles. Se generan desde \codepath{src/neo\_ange/findings} y leen risk scores, simulaciones, grafo, evidencia de modelos y dataset. El archivo \codepath{reports/findings/findings\_summary.json} contiene 19 hallazgos y reporta 4,000 objetos, 982 PHA, 3,018 non-PHA, 18 objetos con señal Sentry, 37 objetos elevados o superiores, 315 filas de simulación de score, 50 filas de simulación orbital, 4,000 nodos de grafo y 27,829 aristas.

Ese reporte no coincide por completo con los Parquet actuales de \codepath{data/gold}, que tienen 1,000 filas en gold/risk y 1,000 nodos de grafo. Por tanto, los findings se documentan como resultado generado existente, no como descripción única del estado actual de todas las capas. Esta limitación es importante para reproducibilidad: antes de presentar métricas definitivas, debe regenerarse el pipeline completo o congelarse un snapshot consistente.

\chapter{API FastAPI}

FastAPI permite exponer rutas HTTP con validación Pydantic \cite{fastapi_docs}. La aplicación vive en \codepath{src/neo\_ange/api/main.py}. Los routers verificados incluyen:
...
Docker empaqueta dependencias y servicios; Docker Compose levanta múltiples servicios desde un solo archivo \cite{docker_ubuntu,docker_debian,docker_compose}. El \codepath{docker-compose.yml} define:
...
\section{Snapshot Verificado}

La Tabla \ref{tab:results} resume resultados leídos localmente con Pandas y archivos JSON.

\begin{table}[H]
\centering
\input{tables/results_snapshot.tex}
...
El ranking actual de \codepath{data/gold/risk\_scores} contiene 1,000 objetos. El objeto con score máximo es \code{20152637}, designación \code{152637}, nombre completo \code{152637 (1997 NC1)}, con score 47.271572 y categoría \code{elevated}. El score promedio verificado en \codepath{reports/risk/risk\_scores\_summary.json} es 28.766259561, con mediana 29.164740000000002, mínimo 14.389905 y máximo 47.271572.

Los resultados de simulación de score muestran que no hay incertidumbre formal completa: \code{formal\_uncertainty\_count=0}. La simulación orbital tiene reportes de 50 filas, pero el Parquet actual contiene una fila. El grafo actual tiene 1,000 nodos y 6,955 aristas; el reporte de findings conserva una corrida de 4,000 nodos y 27,829 aristas. Esta situación no invalida el proyecto, pero sí exige documentar que los artefactos no provienen todos del mismo snapshot.
...
Puerto ocupado & Revisar procesos con \code{sudo ss -ltnp | grep ':8000\|:5174'} o cambiar puertos en \code{docker-compose.yml}.\\
...
En la computadora usada para preparar esta entrega, \code{docker} no está instalado o no está en PATH, por lo que no se pudo ejecutar \code{docker compose up}. Tampoco se compiló PDF porque no se asumió una instalación LaTeX local.
...
El repositorio contiene los siguientes diagramas fuente: \codepath{docs/diagrams/system\_architecture.mmd}, \codepath{docs/diagrams/data\_pipeline\_bronze\_silver\_gold.mmd}, \codepath{docs/diagrams/class\_diagram\_entities.mmd}, \codepath{docs/diagrams/class\_diagram\_system.mmd}, \codepath{docs/diagrams/risk\_scoring\_flow.mmd}, \codepath{docs/diagrams/score\_monte\_carlo\_flow.mmd}, \codepath{docs/diagrams/orbital\_simulation\_flow.mmd}, \codepath{docs/diagrams/ml\_model\_evidence\_flow.mmd}, \codepath{docs/diagrams/gnn\_orbital\_graph\_flow.mmd}, \codepath{docs/diagrams/api\_frontend\_sequence.mmd} y \codepath{docs/diagrams/final\_app\_navigation.mmd}. Este documento incluye la imagen UML disponible y referencia los Mermaid como fuentes auditables.
